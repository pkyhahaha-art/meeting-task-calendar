...
import { Bell, CalendarClock, FileText, Loader2, Mail, Paperclip, Plus, Repeat2, Trash2, UserPlus, X } from 'lucide-react'
import type { Database } from '../lib/database.types'
import { invalidGuestEmails, recurrenceFromRule, reminderOptions, validateAttachments, type Recurrence, type ReminderKey } from '../lib/eventForm'

type EventRow = Database['public']['Tables']['events']['Row']
type AttachmentRow = Database['public']['Tables']['attachments']['Row']
export type EventDetails = { guestEmails: string[]; reminderKeys: ReminderKey[]; notifyEmail: boolean; notifyLine: boolean; attachments: AttachmentRow[] }
export type EventDraft = Pick<EventRow, 'title' | 'description' | 'location' | 'all_day'> & {
  start: string; end: string; recurrence: Recurrence; guestEmails: string[]; reminderKeys: ReminderKey[]
...
const blankDraft = (date?: string): EventDraft => ({
  ...
  start: date ? `${date}T09:00` : '', end: date ? `${date}T10:00` : '',
  recurrence: 'none', guestEmails: [''], reminderKeys: ['1:day'], notifyEmail: true, notifyLine: false, files: [],
})

function localValue(value: string | null, allDay: boolean) {
  if (!value) return ''
  const date = new Date(value)
  if (allDay) return date.toLocaleDateString('en-CA', { timeZone: 'Asia/Bangkok' })
...
export function EventDialog({ open, event, details, selectedDate, canEdit, busy, onClose, onSave, onDelete }: {
  ...
  useEffect(() => {
    setError('')
    setDraft(event ? {
      title: event.title, description: event.description, location: event.location, all_day: event.all_day,
      start: localValue(event.start_datetime, event.all_day), end: localValue(event.end_datetime, event.all_day),
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!draft.title.trim() || !draft.start) return setError('กรุณากรอกชื่อและวันเวลาเริ่ม')
    if (draft.end && new Date(draft.end) < new Date(draft.start)) return setError('เวลาสิ้นสุดต้องไม่ก่อนเวลาเริ่ม')
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/35 p-0 sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="event-title">
      <section className="max-h-[95vh] w-full max-w-3xl overflow-y-auto rounded-t-2xl bg-white p-5 shadow-2xl sm:rounded-2xl sm:p-6">
        <div className="mb-5 flex items-start justify-between"><div className="flex gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-50 text-brand-600"><CalendarClock size={22} /></span><div><h2 id="event-title" className="text-xl font-bold">{event ? 'รายละเอียดการประชุม' : 'เพิ่มการประชุม'}</h2>{event && !canEdit && <p className="text-sm text-slate-500">ดูได้อย่างเดียว เฉพาะเจ้าของเท่านั้นที่แก้ไขได้</p>}</div></div><button type="button" onClick={onClose} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100" aria-label="ปิด"><X size={20} /></button></div>
        <form onSubmit={submit} className="space-y-5">
          <fieldset disabled={!canEdit || busy} className="space-y-5 disabled:opacity-75">
            <section className="space-y-4 rounded-xl border border-slate-200 p-4">
              ...
              <label className="flex items-center gap-2 text-sm font-medium text-slate-700"><input type="checkbox" checked={draft.all_day} onChange={(e) => { set('all_day', e.target.checked); set('start', ''); set('end', '') }} className="h-4 w-4 rounded border-slate-300 text-brand-600" />ทั้งวัน</label>
              <div className="grid gap-4 sm:grid-cols-2"><div><label className="field-label" htmlFor="start">เริ่ม *</label><input id="start" type={draft.all_day ? 'date' : 'datetime-local'} className="field-input" value={draft.start} onChange={(e) => set('start', e.target.value)} /></div><div><label className="field-label" htmlFor="end">สิ้นสุด (ไม่บังคับ)</label><input id="end" type={draft.all_day ? 'date' : 'datetime-local'} className="field-input" value={draft.end} min={draft.start} onChange={(e) => set('end', e.target.value)} /></div></div>
            ...
            <section className="space-y-3 rounded-xl border border-slate-200 p-4"><h3 className="flex items-center gap-2 font-semibold text-slate-800"><Paperclip size={18} className="text-brand-600" />ไฟล์แนบ</h3>{details?.attachments.map((file) => <div key={file.id} className="flex items-center gap-2 rounded-lg bg-slate-50 px-3 py-2 text-sm"><FileText size={16} /><span className="truncate">{file.file_name}</span><span className="ml-auto text-xs text-slate-400">{(file.file_size / 1024 / 1024).toFixed(1)} MB</span></div>)}{draft.files.map((file, index) => <div key={`${file.name}-${index}`} className="flex items-center gap-2 rounded-lg bg-brand-50 px-3 py-2 text-sm"><FileText size={16} /><span className="truncate">{file.name}</span><button type="button" className="ml-auto text-slate-500 hover:text-red-600" onClick={() => set('files', draft.files.filter((_, itemIndex) => itemIndex !== index))} aria-label={`นำ ${file.name} ออก`}><X size={16} /></button></div>)}<input ref={fileInput} type="file" multiple className="hidden" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.jpg,.jpeg,.png" onChange={(e) => { const files = [...(e.target.files ?? [])]; const message = validateAttachments([...draft.files, ...files], details?.attachments.length ?? 0); if (message) setError(message); else { setError(''); set('files', [...draft.files, ...files]) }; e.target.value = '' }} /><button type="button" className="btn-secondary" onClick={() => fileInput.current?.click()}><Paperclip size={17} />เลือกไฟล์</button><p className="text-xs text-slate-500">สูงสุด 5 ไฟล์ ไฟล์ละไม่เกิน 10 MB: PDF, Office, JPG และ PNG</p></section>
          </fieldset>
          {error && <p className="form-error" role="alert">{error}</p>}
          <div className="flex flex-wrap justify-between gap-2 border-t border-slate-100 pt-4">{event && canEdit ? <button type="button" onClick={onDelete} className="btn-secondary border-red-200 text-red-600 hover:bg-red-50" disabled={busy}><Trash2 size={17} />ลบ</button> : <span />}<div className="ml-auto flex gap-2"><button type="button" onClick={onClose} className="btn-secondary">{canEdit ? 'ยกเลิก' : 'ปิด'}</button>{canEdit && <button className="btn-primary" disabled={busy}>{busy && <Loader2 className="animate-spin" size={17} />}{event ? 'บันทึกการแก้ไข' : 'สร้างการประชุม'}</button>}</div></div>
        </form>
      </section>
    </div>
  )
