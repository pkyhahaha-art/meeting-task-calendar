import { ArrowLeft, CalendarDays, LogOut, Settings, ShieldCheck, UserRound } from 'lucide-react'
    ...
    <div className="min-h-screen bg-slate-50">
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex min-h-16 max-w-[1600px] flex-wrap items-center gap-3 px-4 py-2 sm:px-6">
          ...
          {location.pathname !== '/calendar' && <button type="button" onClick={() => navigate('/calendar')} className="btn-secondary px-2.5 sm:px-3" aria-label="กลับไปหน้าปฏิทิน"><ArrowLeft size={17} /><span className="hidden sm:inline">กลับ</span></button>}
          <div className="ml-auto flex items-center gap-1">
            <LanguageToggle />
            <div className="hidden shrink-0 items-center gap-2 px-2 text-right md:flex"><UserRound size={18} className="text-slate-400" /><div><div className="flex flex-wrap items-center justify-end gap-1.5"><p className="whitespace-nowrap text-sm font-semibold">{profile?.full_name || user?.email}</p>{profile?.role === 'admin' && <span className="rounded-full bg-violet-100 px-2 py-0.5 text-[10px] font-bold text-violet-700">ผู้ดูแลระบบ</span>}</div><p className="text-xs text-slate-500">{profile?.employee_id || '—'}</p></div></div>
